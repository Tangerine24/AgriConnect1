...
const TrendAnalyzer_1 = require("./engine/TrendAnalyzer");
const app = (0, express_1.default)();
const prisma = new client_1.PrismaClient();
const decisionEngine = new DecisionEngine_1.DecisionEngine();
const trendAnalyzer = new TrendAnalyzer_1.TrendAnalyzer();
...
app.post('/api/recommendations', async (req, res) => {
    ...
    for (const m of markets) {
        ...
        const explanations = [];
        if (m.distance < 100)
            explanations.push(`Close proximity (${m.distance} km) reduces transport cost`);
        else if (m.distance < 300)
            explanations.push(`Moderate distance (${m.distance} km)`);
        else
            explanations.push(`Distant market (${m.distance} km) — higher transport cost`);
        if (price.modalPrice >= price.maxPrice * 0.95)
            explanations.push('Modal price near the market high');
        if (enr.transportCost < enr.expectedSellingPrice * 0.05)
            explanations.push('Transport cost is less than 5% of revenue');
    ...
    for (const b of buyers) {
        if (avgModalPrice === 0)
            continue; // No price data for this commodity
        ...
        if (maxDistance && buyerDistance > maxDistance)
            continue;
        ...
        if (b.reliabilityScore > 90)
            score += 500;
        if (b.reliabilityScore > 80)
            score += 200;
        if (isUnverified)
            score -= 2000;
        const explanations = [];
        if (!isUnverified)
            explanations.push('Verified buyer — lower transaction risk');
        else
            explanations.push('Unverified buyer — higher risk, 5% price penalty applied');
        if (b.reliabilityScore >= 85)
            explanations.push(`High reliability score (${b.reliabilityScore}/100)`);
        else if (b.reliabilityScore >= 60)
            explanations.push(`Moderate reliability (${b.reliabilityScore}/100)`);
        else
            explanations.push(`Low reliability (${b.reliabilityScore}/100) — limited history`);
        if (b.completedTransactions > 50)
            explanations.push(`${b.completedTransactions} completed transactions`);
